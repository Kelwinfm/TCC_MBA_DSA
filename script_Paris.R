...
Paris_reviews <- read_csv("Paris_reviews.csv")
...
review_3 <- Paris_reviews %>%  filter(rating_review == "3")
...
ggplot(tabela_relevante, aes(x = word)) +
  geom_bar(aes(y = total), stat = "identity", position = "dodge", fill = "green", width = 0.5) +
  geom_bar(aes(y = with_not), stat = "identity", position = "dodge", fill = "darkorange", width = 0.5) +
  labs(title = "Quantidades de Palavras total x Palavras precedidas com negativa",
       x = "Palavra",
       y = "Quantidade") +
  scale_fill_manual(values = c("Total de Palavras" = "green", "Palavras com negação" = "darkorange"), name = "Legenda de Cores") +
  theme_minimal()

#limitar a tabela para construção de grafico
tabela_limitada <- tabelas_unidas %>% 
  filter(total > 2100)%>%
  arrange(desc(total))


ggplot(tabela_limitada, aes(x = word)) +
  geom_bar(aes(y = total), stat = "identity", position = "dodge", fill = "green", width = 0.5) +
  geom_bar(aes(y = with_not), stat = "identity", position = "dodge", fill = "darkorange", width = 0.5) +
  labs(title = "Quantidades de Palavras total x Palavras precedidas com negativa",
       x = "Palavra",
...
#plotar grafico das palavras acompanhadas de "not"
not_words_positive %>%
...
review_positive <- Paris_reviews %>%  filter(sample == "Positive")
...
filtro_mais_avaliados2 <- Paris_reviews
filtro_mais_avaliados2 <- filtro_mais_avaliados2[, !names(filtro_mais_avaliados2) %in% c("parse_count", "review_id","sample", "review_preview","review_full", "city", "url_restaurant", "author_id")]

filtro_mais_avaliados2 <- filtro_mais_avaliados2 %>%
  group_by(restaurant_name) %>% 
  summarise(
    qtd_reviews = n(), 
    media_notas = mean(rating_review, na.rm = TRUE),
...
media_5_P <- filtro_mais_avaliados2 %>%
  filter(media_notas == 5) %>%
  select(restaurant_name,qtd_reviews,- media_notas, -variacao_notas, -variancia) %>%
  filter(qtd_reviews > 12) %>%
  rename(Nome_Restaurante_Paris = "restaurant_name") %>%
  rename(Quantidade_comentarios_Paris = "qtd_reviews")

#ordenar em forma decrescente de comentarios
media_5_P <- media_5_P[order(media_5_P$Quantidade_comentarios_Paris, decreasing = TRUE),]
...
filtro_grafico <- filtro_mais_avaliados2 %>%
  filter (media_notas > 4, qtd_reviews > 1000)
